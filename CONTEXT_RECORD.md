...
- **Two complementary "past data" sources:**
  - **Historical baseline (months) — the reference:** a real punctuality *archive* from [piebro/deutsche-bahn-data](https://github.com/piebro/deutsche-bahn-data) (DB data, CC BY 4.0). Pre-aggregated once into compact arrival-delay KPIs per `(station EVA, train_type)` (`journey_autopilot/disruption_monitoring/build_db_delay_reference.py` → `journey_autopilot/data/db_delay_reference.json`, ~370 kB). Runtime reads only the JSON — no heavy deps, works offline (EVA via sidecar, else a name index). This is the long-run normal case the score is anchored on.
...
#### Decisions
- **Communication is the Communicator Agent's job (implemented):** the Drafter (`whatsapp_communicator/drafter.py`) drafts role-appropriate messages; `whatsapp_communicator/tools.py` sends them via Twilio with an approval queue (5-min timeout). The Communicator is also the channel through which the user's veto comes back (YES/NO/EDIT via the webhook). It brackets execution: before the veto it presents options and captures the decision; after execution it confirms to traveler and meeting participants.
