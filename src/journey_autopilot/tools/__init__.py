...
- ``write_tools`` — side-effectful, gated by the policy layer. Implemented:
                    the notice-email propose/approve/send pair (the
                    ``send_email_to_participants`` action, policy "ask");
                    further write tools per build spec M4.
