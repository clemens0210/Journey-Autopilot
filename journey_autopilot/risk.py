...
from .tools import get_connection_delay_history, get_planned_connection
...
1. Hole mit `get_connection_delay_history` die Pünktlichkeits-Historie der
   Verbindung (origin, destination, ggf. train). Das ist deine Datenbasis:
   mittlere/median/p90-Verspätung, Pünktlichkeitsquote, Ausfälle, Ursachen.
2. Hole mit `get_planned_connection` die geplante Soll-Ankunft als ETA-Anker.
3. Leite daraus ab:
   - **Erwartete Verspätung**: nimm die `median_delay_minutes` als typischen Wert
     und die `p90_delay_minutes` als ungünstigen (realistischen Worst-Case).
...
- Datenbasis: <sample_count> Fahrten, Quelle <db_service_live|mock_history>, <window>
- Begründung: 1–2 Sätze (nenne Pünktlichkeitsquote und die Hauptursachen)
...
- Liefert ein Tool `error` oder kein Sample, sag das offen und gib nur das aus, was
  belastbar ist (keine ETA ohne Soll-Ankunft).
...
def build_risk_agent() -> LlmAgent:
    ...
    return LlmAgent(
        ...
        tools=[get_connection_delay_history, get_planned_connection],
    )
